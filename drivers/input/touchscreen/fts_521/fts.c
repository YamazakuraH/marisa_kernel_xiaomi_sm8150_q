...
static int fts_mode_handler(struct fts_ts_info *info, int force)
{
	...
	case 1:
		...
		info->fod_enabled = 0;// Always make the reader think fingerprint isn't enabled
...
static int fts_probe(struct spi_device *client)
{
	...
	info->fod_status = 1; // Always enable the fingerprints
